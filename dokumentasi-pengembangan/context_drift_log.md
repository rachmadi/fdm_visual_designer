# 📉 Context Drift Log — FDM Visual Designer
## Format: Kumulatif per Iterasi | Spesifikasi: Revisi 3 Final

> File ini mencatat setiap penyimpangan (drift) dari spesifikasi awal yang terjadi selama sesi pengembangan.
> Context drift adalah kondisi di mana implementasi mulai bergeser dari rencana awal tanpa keputusan eksplisit.

---

## Cara Mengisi

```
Tambahkan blok baru untuk setiap drift yang terdeteksi.
Satu blok per kejadian drift.
```

---

═══════════════════════════════════════════════════════════════════
## ITERASI 1a — Fondasi Canvas & Three-Node Architecture
### Tanggal: 2026-07-06
═══════════════════════════════════════════════════════════════════

### CD-001
- **Waktu Terdeteksi**: 2026-07-06 10:29 WIB
- **Lokasi**: `lib/canvas/canvas_view.dart`
- **Spesifikasi Asli**: InteractiveViewer + inversi matriks untuk transformasi koordinat
- **Implementasi Aktual**: Sempat menggunakan `Listener` gesture manual untuk mengontrol zoom/pan secara kustom.
- **Penyebab Drift**: Kekhawatiran awal mengenai tabrakan event gesture antara penyeretan node dan canvas panning.
- **Dampak**: Pengelolaan koordinat menjadi kompleks, rentan bug hit-test, dan tidak sesuai spesifikasi dasar.
- **Resolusi**: Di-refaktor kembali ke arsitektur `InteractiveViewer` + `TransformationController` + inversi matriks (`Matrix4.inverted()`) dengan penonaktifan `panEnabled`/`scaleEnabled` dinamis saat drag.
- **Status**: ✅ Teratasi / Kembali ke spesifikasi asli

---

*[Tambahkan entri baru di bawah ini saat iterasi 1a berjalan]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 1b — [Template untuk iterasi berikutnya]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 1b dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 2a — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 2a dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 2b — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 2b dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 3a — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 3a dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 3b — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 3b dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 4a — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 4a dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 4b — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok ini akan diisi saat Iterasi 4b dimulai]*

---

═══════════════════════════════════════════════════════════════════
## ITERASI 5a–7 — [Template]
═══════════════════════════════════════════════════════════════════

*[Blok-blok ini akan diisi pada iterasi yang sesuai]*

---

*Dokumen ini dibuat: 2026-07-06 | Diperbarui: otomatis setiap akhir iterasi*
