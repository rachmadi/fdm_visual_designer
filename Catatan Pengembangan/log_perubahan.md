...
- **Penyelesaian Konflik Gesture Drag & Pan Canvas**: Menonaktifkan sementara pan/zoom `InteractiveViewer` kanvas ketika kursor mouse melayang (`hover`) di atas node atau boundary menggunakan `nodeHoverProvider`. Ini mencegah `InteractiveViewer` merampas gesture seret, sehingga setiap node dapat digeser secara individu secara independen.
